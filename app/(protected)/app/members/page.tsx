            ...
            <div className="divide-y divide-border">
              {pendingInvites.map((invite) => (
                <div key={invite.id} className="flex items-center gap-3 py-3">
                  <Avatar className="h-9 w-9">
                    <AvatarFallback className="text-xs">{invite.email.slice(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                  ...
                  <Badge variant="secondary" className="text-xs capitalize">{invite.role.toLowerCase()}</Badge>
                </div>
              ))}
            </div>
          </CardContent>
