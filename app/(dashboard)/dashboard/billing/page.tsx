        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>{company?.name ?? 'Your workspace'}</CardDescription>
            </div>
            <Badge variant={currentPlan === 'FREE' ? 'secondary' : 'default'}>{planInfo.name}</Badge>
          </div>
        </CardHeader>
        <CardContent>
              ...
              <CardContent>
                ...
                {!isActive && plan.price > 0 && (
                  <Button size="sm" className="w-full" asChild>
                    <Link href={`/dashboard/billing/upgrade?plan=${key}`}>Upgrade</Link>
                  </Button>
                )}
                {!isActive && plan.price === 0 && currentPlan !== 'FREE' && (
                  <Button size="sm" variant="outline" className="w-full">Downgrade</Button>
                )}
              </CardContent>
            </Card>
          )
