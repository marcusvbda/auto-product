	...
	if (invite.expiresAt < new Date()) {
		return (
			<InviteError
				...
				description="This invitation has expired. Ask the workspace owner to send a new invite."
			/>
		);
	}
						...
						<div className="space-y-3">
							...
							<div className="flex flex-col gap-2">
								...
								<Button variant="outline" asChild>
									<Link
										href={`/register?email=${encodeURIComponent(invite.email)}&redirect=/invite?token=${token}`}
									>
										Create account
									</Link>
								</Button>
